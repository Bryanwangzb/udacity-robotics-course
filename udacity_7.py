...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if alpha<0.001:
          x1 = self.x + d*cos(self.orientation)
          y1 = self.y + d*sin(self.orientation)
          o1 = self.orientation
        else:
          ...
          x1 = cx + sin(beta + self.orientation)*R
          y1 = cy - cos(beta + self.orientation)*R
          o1 = self.orientation+beta
          while (o1>=2*pi):
            o1 -= 2*pi
    ...
    # according to
    #           self.bearing_noise
    def sense(self, use_noise=0): #do not change the name of this function
        Z = []

        for i in range(len(landmarks)):
          angle = atan2(landmarks[i][0]-self.y,landmarks[i][1]-self.x) - self.orientation 
          if use_noise != 0:
            angle += random.gauss(0, self.bearing_noise)
          while angle<0:
            angle+= 2*pi
          Z.append( angle )
